...
CEcgModule::CEcgModule( QWidget *parent)
{
    ...
    m_port = new Posix_QextSerialPort(ECG_MODULE_DATA_PORT_NAME,QextSerialBase::Polling);
...
*/
    ...
    m_modu_data_file.open(QIODevice::ReadOnly);
...
void CEcgModule::f_plot_ecg_wave(int data1,int data2,int data3)
{
    ...
    if(m_ecg_cfg->m_ecg_wave_speed == 0) //3.125mm/s
    {
        x = g_PixPerByte_3_125mm;
    }
    else if(m_ecg_cfg->m_ecg_wave_speed == 1) // 6.25mm/s
    {
        x = g_PixPerByte_6_25mm;
    }
    else if(m_ecg_cfg->m_ecg_wave_speed == 2) // 12.5mm/s
    {
...
void CEcgModule::f_set_ecg_def_cfg()
{
    ...
    for(int i=0;i<ECG_MODULE_WAVE_NUM;i++)
    {
        ...
        m_ecg_cfg->m_wave_rec[i].setRect(0,50*i+400,730,50);
            ...
            case 0x05://心电波形
                ...
                (m_para_data_one_buf[8] & 0x7f)))
                {
                    ...
                    //emit signal_setpanels_value(pMainForm->TextLabel_bar_alarm_II,ch1);
                    {
                        ...
                        {
                            char cnt = m_ecg_wave_count*255/499;
...
/*                        else if(m_spire_wave_count >= 125)
                        {
                            bFlg = true;
                            m_ecg_wave_count =0;
                            m_spire_wave_count = 0;
                            ...
                            emit signal_para_tcp_data_ready(data,1); //send msg
                            //存入暂存的数据
            ...
            case 0x10://
                ...
                (m_para_data_one_buf[3] & 0x7f)))
                {
                    ...
                    {
                        char cnt = m_spire_wave_count*127/124;
                        m_ecg_val.m_spire_wave_data[cnt] = value;
                        m_ecg_value[ECG_VAL_SPIRE_WAVE_C+cnt] = value;
                        m_spire_wave_count++;
                    }
 /*                   else if((m_ecg_wave_count >=500))
                    {
                        //QByteArray data;
                        //data.append((char *)&m_spire_val,278);
                        m_spire_wave_count = 0;
                        m_ecg_wave_count = 0;
            ...
            case 0x15://
                ...
                (m_para_data_one_buf[7] & 0x7f)))
                {
                    ...
                    else
                   {
                    ...
                    m_temp_val.m_para_temp1 = iTempVal1;
                    m_temp_val.m_para_temp2 = iTempVal2;
            ...
            case 0x21://
                ...
                (m_para_data_one_buf[3] & 0x7f)))
                {
                    ...
                    if ((m_para_data_one_buf[2] & 0x7f) < 3)
                    {
                        ...
                        m_nibp_val.m_nibp_over_flg = true;
                    }
                    bNibpStat= FALSE;
                    //emit single_for_QLabel_set(TextLabel_WuChuangXueYa_xiudaiya_value,"",type_hide);
                    //emit single_for_QLabel_set(TextLabel_WuChuangXueYa_xiudaiya,"",type_hide);
...
/*
        _FY0803_baojing_jishu[1].tiaojian=0;
        _FY0803_baojing_jishu[0].tiaojian=ucLeadStat[0];
        _FY0803_baojing_jishu[2].tiaojian=((spo2zhuangtai&0x10)>>3)|((spo2zhuangtai&0x80)>>7);
        _FY0803_baojing_jishu[3].tiaojian=iTempStat;
        _FY0803_baojing_jishu[5].tiaojian=   iNibpErr;

        int reco=0;
        static int  ECGWAVE_Calculate_record[3];
        int num=BY_ECG_buff[0].size()/4;

        for( int y=0;y<(num);y++)
        {
            ECGWAVE_Calculate_record[0]=(uchar)(BY_ECG_buff[0].at(reco));
            ECGWAVE_Calculate_record[1]=(uchar)(BY_ECG_buff[1].at(reco));
            ECGWAVE_Calculate_record[2]=(uchar)(BY_ECG_buff[2].at(reco));

            for(uchar z=1;z<4;z++)
            {
                if(ECGWAVE_Calculate_recordCMainForm[0 ]<(uchar)(BY_ECG_buff[0].at(reco+z)))
                    ECGWAVE_Calculate_record[0 ]=(uchar)(BY_ECG_buff[0].at(reco+z));

                if(ECGWAVE_Calculate_record[1 ]<(uchar)(BY_ECG_buff[1].at(reco+z)))
                    ECGWAVE_Calculate_record[1 ]=(uchar)(BY_ECG_buff[1].at(reco+z)) ;

                if(ECGWAVE_Calculate_record[2 ]<(uchar)(BY_ECG_buff[2].at(reco+z)))
                    ECGWAVE_Calculate_record[2 ]=(uchar)(BY_ECG_buff[2].at(reco+z)) ;
            }
            reco+=4;
            if(!XinDianXiaoZhun)
            {
                DRAW_ECG_WAVE[0][iEcgWavCnt]=(ECGWAVE_Calculate_record[0]-128)>>2;///10/;
                DRAW_ECG_WAVE[1][iEcgWavCnt]=(ECGWAVE_Calculate_record[1]-128)>>2;///10/;
                DRAW_ECG_WAVE[6][iEcgWavCnt]=(ECGWAVE_Calculate_record[2]-128)>>2;///10/;

                DRAW_ECG_WAVE[2][iEcgWavCnt]=DRAW_ECG_WAVE[1][iEcgWavCnt]-DRAW_ECG_WAVE[0][iEcgWavCnt];
                DRAW_ECG_WAVE[3][iEcgWavCnt]=-(DRAW_ECG_WAVE[1][iEcgWavCnt]+DRAW_ECG_WAVE[0][iEcgWavCnt])/2;
                DRAW_ECG_WAVE[4][iEcgWavCnt]=DRAW_ECG_WAVE[1][iEcgWavCnt]-DRAW_ECG_WAVE[0][iEcgWavCnt]/2;
                DRAW_ECG_WAVE[5][iEcgWavCnt]=DRAW_ECG_WAVE[0][iEcgWavCnt]-DRAW_ECG_WAVE[1][iEcgWavCnt]/2;
            }
            else
            {
                for(int ii=0;ii<7;ii++)
                DRAW_ECG_WAVE[ii][iEcgWavCnt]=(ECGWAVE_Calculate_record[0]-128)>>2;
            }
//            cccccc++;
            iEcgWavCnt++;
        }
        pthread_mutex_lock(&mutex_TCP_ByteArray);
        ByteArray_SPO2WAVE.append((char*)qbSpo2Wav,iSpo2WavCnt);
        ByteArray_RESPWAVE.append((char*)qbSireWav,iSpireWavCnt);
        ByteArray_ECGWAVE_ECG1.append(BY_ECG_buff[0].data(),reco);
        ByteArray_ECGWAVE_ECG2.append(BY_ECG_buff[1].data(),reco);
        ByteArray_ECGWAVE_ECG7.append(BY_ECG_buff[2].data(),reco);
        for(uchar y=0;y<3;y++)
        {
            BY_ECG_buff[y].remove(0,reco);
        }
