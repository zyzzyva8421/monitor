...
void CDrawThread::f_draw_ana(int waveindex, const QVector<QPointF> &data,int iStop)
{
    for(int i = 0;i<data.count();i++)
    {
        anadata[anawrite].setX( data[i].x());
        anadata[anawrite].setY(data[i].y());
        anaindx[anawrite] = waveindex;
        anastop[anawrite] = iStop;
        anawrite++;
        anawrite %= DRAW_DATA_MAX;
    }
}
...
///////////////////////////////////////////////////////////////////////////////
        ...
        QVector<int> istop;
        istop.resize(MAX_PLOT_CURVES); // 最大波形数量为 .
        int pwrite = ecgwrite;        
        if(ecgread != pwrite)
        {
            QVector<QPointF> tempint;
            ...
            int idx = ecgindx[ecgread];
            curveMap[idx] = tempint;
            istop[idx] = ecgstop[ecgread];
        ...
        if(spo2read != pwrite)
        {
            ...
            int idx = spo2indx[spo2read];
            curveMap[idx] = tempint;
            istop[idx] = spo2stop[spo2read];
        ...
        if(spireread != pwrite)
        {
            ...
            int idx = spireindx[spireread];
            curveMap[idx] = tempint;
            istop[idx] = spirestop[spireread];
        ...
        if(eegread != pwrite)
        {
            ...
            int idx = eegindx[eegread];
            curveMap[idx] = tempint;
            istop[idx] = eegstop[eegread];
        ...
        if(co2read != pwrite)
        {
            ...
            int idx = co2indx[co2read];
            curveMap[idx] = tempint;
            istop[idx] = co2stop[co2read];
        ...
        {
            QVector<QPointF> tempint;
            tempint.append(anadata[anaread]);
            //g_WavePlotter->setCurveData(co2indx[co2read], tempint,co2stop[co2read]);

            int idx = anaindx[anaread];
            curveMap[idx] = tempint;
            istop[idx] = anastop[anaread];

            anaread++;
            anaread %= DRAW_DATA_MAX;
        }

        g_WavePlotter->setCurveDatas(curveMap,istop);
        usleep(2);
    }
    this->exec();
}
