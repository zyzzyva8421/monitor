...
void WavePlotter::setCurveDatas(int  len,QMap<int, QVector<QPointF> > &data,int iStop)
{
    curveMap = data;
    iStopPos = iStop;
    iDrawLen = len;
...
//               width() - 2 * Margin, height() - 2 * Margin);
 ...
 /*           double dx = data[j].x() - settings.minX;
        ...
        painter->setPen(Qt::black);
        painter->setBrush(QBrush(Qt::black,Qt::SolidPattern));
        painter->drawRect(iStopPos,(0+id*45),10,50);
        painter->setPen(wavecolors[id]);
        painter->drawStaticText(20,(32+id*45),wavenames[id]);

 /*          iStopPos = rect.left()+
                (iLastX-settings.minX)* (rect.width() - 1)/ settings.spanX();
     int iSeparate = 0;
        int j = 0;
        for (j = 0; j < data.count(); ++j) {
            double dx = data[j].x() - settings.minX+iLastX;
            double dy = data[j].y() - settings.minY;

            double y = rect.bottom() - (dy * (rect.height() - 1)
                                           / settings.spanY());
            //get Y
            if(y>rect.height()) //边界线
            {
                y = rect.height();
            }
            //get x
            ...
            {
                polyline[j-iSeparate] = QPointF(x, y);
            }
            iSotpPos = x;
        }
        iLastX += (j-iSeparate);
